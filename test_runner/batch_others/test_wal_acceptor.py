...
def test_wal_removal(zenith_env_builder: ZenithEnvBuilder, auth_enabled: bool):
    zenith_env_builder.num_safekeepers = 2
    # to advance remote_consistent_llsn
    zenith_env_builder.enable_local_fs_remote_storage()
    zenith_env_builder.auth_enabled = auth_enabled
    env = zenith_env_builder.init_start()
    ...
    if auth_enabled:
        ...
        # TODO: ensure the record_safekeeper_info was not executed
...
@pytest.mark.parametrize('auth_enabled', [False, True])
def test_timeline_status(zenith_env_builder: ZenithEnvBuilder, auth_enabled: bool):
    zenith_env_builder.auth_enabled = auth_enabled
    env = zenith_env_builder.init_start()
...
@pytest.mark.parametrize('auth_enabled', [False, True])
def test_delete_force(zenith_env_builder: ZenithEnvBuilder, auth_enabled: bool):
